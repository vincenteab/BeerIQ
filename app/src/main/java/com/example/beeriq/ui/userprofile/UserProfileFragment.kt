...
class UserProfileFragment : Fragment(R.layout.fragment_userprofile) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        radioGenres = view.findViewById(R.id.Radio_Gender_Option)
        ...
        Util.checkPermissions(requireContext())
...
//            finish()
        ...
        cameraResult = registerForActivityResult(ActivityResultContracts.StartActivityForResult()){
            ...
            if (result.resultCode == Activity.RESULT_OK){
                val bitmap = Util.getBitmap(requireContext(), imgUri)
                imageView.setImageBitmap(bitmap)
                view.findViewById<ImageView>(R.id.profile_image).setImageURI(imgUri)
                val imageFile = File(requireContext().getExternalFilesDir(Environment.DIRECTORY_PICTURES), tempImgFileName)
...
//            snackBarMessage("Gender not selected", R.color.red)
    ...
    private fun openCamera() {
        val imageFile = File(requireContext().getExternalFilesDir(Environment.DIRECTORY_PICTURES), tempImgFileName)
        imgUri = FileProvider.getUriForFile(requireContext(), "${requireContext().packageName}.fileprovider", imageFile)
    ...
    private fun saveFormData() {
        ...
        val gender = when {
            view?.findViewById<RadioButton>(R.id.Radio_Male)?.isChecked == true -> "Male"
            view?.findViewById<RadioButton>(R.id.Radio_Female)?.isChecked == true -> "Female"
            else -> ""
        }
    ...
    private fun loadFormData() {
        ...
        when (gender) {
            "Male" -> view?.findViewById<RadioButton>(R.id.Radio_Male)?.isChecked = true
            "Female" -> view?.findViewById<RadioButton>(R.id.Radio_Female)?.isChecked = true
        }
    }
